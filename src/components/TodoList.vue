  ...
  methods: {
    ...
    async getTodoList (page = 1, filter = this.filterStatus, backToFirst) {
      ...
      try {
        ...
        console.log(err)
        this.$message.error({ message: '获取列表失败', duration: 1500 })
      }
    },
    completeTodo (item) {
      return axios.post(`/api/todo/done/${item._id}`, item)
        .then(res => {
          if (res.data.success) {
            this.getTodoList(this.page, this.filterStatus, false)
            return this.$message({ message: '操作成功', type: 'success', duration: 1500 })
          }
          this.$message.error({ message: '操作失败', duration: 1500 })
        })
        .catch(err => {
          console.log(err)
          this.$message.error({ message: '操作失败', duration: 1500 })
    ...
    updateTodoDetail (item) {
      return axios.post('/api/todo/update', item)
        .then((res) => {
          if (res.data.success) {
            this.getTodoList(this.page, this.filterStatus, false)
            return this.$message({ message: '更改内容成功', type: 'success', duration: 1500 })
          }
          this.$message.error({ message: '更改内容失败', duration: 1500 })
        })
        .catch((err) => {
          console.log(err)
          this.$message.error({ message: '更改内容失败', duration: 1500 })
